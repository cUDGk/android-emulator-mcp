...
| `tap` | 座標指定でタップ |
| `type_text` | テキスト入力（ASCIIは `input text` で直接入力。日本語等の非ASCIIはデバイスに ADBKeyBoard IME が必要、未インストールならエラー）。`clear_first=true` でフォーカス中フィールドを Ctrl+A → DEL でクリアしてから入力 |
...
注意: `batch` の action 名は単体ツールと一部異なる（`type` ⇔ `type_text`、`key` ⇔ `press_key`）。利用可能な action: `tap`, `find_and_tap`, `type`, `key`, `swipe`, `sleep`, `shell`。
